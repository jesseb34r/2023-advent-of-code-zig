...
const HandType = enum(u8) {
    ...
    pub fn parse(cards: [5]Card) !HandType {
        ...
        var hand_type: HandType = pre_joker: {
            var has_three = false;
            var pairs: u8 = 0;

            for (counts) |count| {
                switch (count) {
                    5 => break :pre_joker .FiveOfAKind,
                    4 => break :pre_joker .FourOfAKind,
                    3 => has_three = true,
                    2 => pairs += 1,
                    else => {},
                }
            ...
            if (has_three and pairs == 1) break :pre_joker .FullHouse;
            if (has_three) break :pre_joker .ThreeOfAKind;
            if (pairs == 2) break :pre_joker .TwoPair;
            if (pairs == 1) break :pre_joker .OnePair;
            break :pre_joker .HighCard;
        };
